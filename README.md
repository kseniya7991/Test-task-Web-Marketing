...
2. В проекте есть 3 калссовых компонента: card, navigation, section. Card и Navigation используются для создания экземляторов карточек товаров / ссылки навигации через шаблон template в html. Далее эти экземпляры карточек рендерятся в нужных блоках через Section.
